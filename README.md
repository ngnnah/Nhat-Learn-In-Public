# 📚 Learn In Public

> *Capturing what I learn each week - a personal journey of continuous growth.*

[![Weekly Updates](https://img.shields.io/badge/Updates-Weekly-brightgreen)](https://github.com/ngnnah/Nhat-Learn-In-Public)

---

## 📅 Weekly Entries

### 2026

| Week    | Date Range      | Summary                                                        | Link                                   |
| ------- | --------------- | -------------------------------------------------------------- | -------------------------------------- |
| Week 05 | Feb 09 - Feb 15 | TBD                                                            | [View →](weeks/2026/week-05/README.md) |
| Week 04 | Feb 02 - Feb 08 | Monty Hall probability & Nanja Monja card game                 | [View →](weeks/2026/week-04/README.md) |
| Week 03 | Jan 26 - Feb 01 | Forecasting principles & calisthenics fitness                  | [View →](weeks/2026/week-03/README.md) |
| Week 02 | Jan 19 - Jan 25 | Built 3 apps (2 math games + GitHub Todo); started MITx 6.431x | [View →](weeks/2026/week-02/README.md) |
| Week 01 | Jan 12 - Jan 18 | Getting started with the learning journal                      | [View →](weeks/2026/week-01/README.md) |

---

## 🏷️ Topics Covered

Each week covers up to 6 life domains:

- 💻 **AI, Data & Programming** — Technical skills, coding, tools
- 💰 **Finance & Money** — Personal finance, investing, economics
- ✈️ **Travel & Culture** — Experiences, languages, perspectives
- 🌱 **Personal Development** — Productivity, habits, growth
- 💪 **Health & Fitness** — Exercise, nutrition, wellness
- ❤️ **Relationships & Life** — Family, connections, balance

---

## 📈 Progress

- **Weeks Documented**: 5
- **Started**: January 2026

---

## 🤖 AI Assistant Setup

Symlinks share instructions between Claude Code and GitHub Copilot:

| Symlink             | Target                            | Purpose                                    |
| ------------------- | --------------------------------- | ------------------------------------------ |
| `.claude/CLAUDE.md` | `.github/copilot-instructions.md` | Single source of truth for AI instructions |
| `.claude/skills`    | `.github/skills`                  | Shared modular skills                      |

**Scripts:**
- `scripts/new-week.sh` — Creates new week folder with sequential numbering
- `scripts/check-urls.sh` — Validates all markdown links

---

<div align="center">

*Keep learning, keep growing, keep sharing.* 🚀

</div>
